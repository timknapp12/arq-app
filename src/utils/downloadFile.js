...
export const downloadFile = async (
  ...
  const callback = (downloadProgress) => {
    ...
    if (contentType === 'image') {
      setToastInfo(filename, `Downloading to Photos...`, true, progress);
    } else {
      setToastInfo(filename, `Downloading...`, true, progress);
    }
    return progress;
  };
  ...
  if (contentType === 'image') {
    setTimeout(() => {
      setToastInfo(filename, `Saved to Photos`, false);
    }, 1000);
    ...
    setTimeout(() => {
      setToastInfo(filename, `Download Complete!`, false);
    }, 1000);
    try {
